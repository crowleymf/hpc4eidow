...
INTEGER bincheck
INTEGER point, loop
DOUBLE PRECISION, ALLOCATABLE:: dxcom(:), dycom(:), dzcom(:)   
               
DOUBLE PRECISION parasquared, perpsquared      
DOUBLE PRECISION, ALLOCATABLE:: etepara(:), eteperp(:)
DOUBLE PRECISION eteboxtot, eteparatot, eteperptot
DOUBLE PRECISION etebox, eteparabox, eteperpbox
DOUBLE PRECISION eteboxtemp,eteparaboxtemp, eteperpboxtemp      
DOUBLE PRECISION eteboxsum, eteparaboxsum, eteperpboxsum
DOUBLE PRECISION eteboxsumdev, eteparaboxsumdev, eteperpboxsumdev
DOUBLE PRECISION etefinal, eteparafinal, eteperpfinal
DOUBLE PRECISION etedev, eteparadev, eteperpdev 
DOUBLE PRECISION maxete

DOUBLE PRECISION, ALLOCATABLE:: etebintot(:), eteparabintot(:), eteperpbintot(:) 
DOUBLE PRECISION, ALLOCATABLE:: etebin(:), eteparabin(:), eteperpbin(:)
DOUBLE PRECISION etebintemp, eteparabintemp, eteperpbintemp
DOUBLE PRECISION, ALLOCATABLE:: etebinsum(:), eteparabinsum(:), eteperpbinsum(:)
DOUBLE PRECISION, ALLOCATABLE:: etebinsumdev(:),eteparabinsumdev(:), eteperpbinsumdev(:)
DOUBLE PRECISION etebinfinal, eteparabinfinal,eteperpbinfinal
DOUBLE PRECISION etebindev, eteparabindev, eteperpbindev

DOUBLE PRECISION dxdiff, dydiff, dzdiff
DOUBLE PRECISION rogsqtot,rogsq
DOUBLE PRECISION, ALLOCATABLE:: rog(:)
DOUBLE PRECISION rogtot
DOUBLE PRECISION rogbox
DOUBLE PRECISION rogboxtemp
DOUBLE PRECISION rogboxsum
DOUBLE PRECISION rogboxsumdev
DOUBLE PRECISION rogfinal
DOUBLE PRECISION rogdev

DOUBLE PRECISION, ALLOCATABLE:: rogbintot(:)
DOUBLE PRECISION, ALLOCATABLE:: rogbin(:)
DOUBLE PRECISION rogbintemp
DOUBLE PRECISION, ALLOCATABLE:: rogbinsum(:)
DOUBLE PRECISION, ALLOCATABLE:: rogbinsumdev(:)
DOUBLE PRECISION rogbinfinal
DOUBLE PRECISION rogbindev

DOUBLE PRECISION, ALLOCATABLE:: thetax(:), thetay(:), thetaz(:)
DOUBLE PRECISION thetaxtot, thetaytot, thetaztot
DOUBLE PRECISION thetaxbox, thetaybox, thetazbox
DOUBLE PRECISION thetaxboxtemp, thetayboxtemp, thetazboxtemp
DOUBLE PRECISION thetaxsum, thetaysum, thetazsum
DOUBLE PRECISION thetaxsumdev, thetaysumdev, thetazsumdev
DOUBLE PRECISION thetaxfinal, thetayfinal, thetazfinal
DOUBLE PRECISION thetaxdev, thetaydev, thetazdev

DOUBLE PRECISION, ALLOCATABLE:: thetaxbintot(:), thetaybintot(:), thetazbintot(:) 
DOUBLE PRECISION, ALLOCATABLE:: thetaxbin(:), thetaybin(:), thetazbin(:) 
DOUBLE PRECISION thetaxbintemp, thetaybintemp, thetazbintemp   
DOUBLE PRECISION, ALLOCATABLE:: thetaxbinsum(:), thetaybinsum(:), thetazbinsum(:)
DOUBLE PRECISION, ALLOCATABLE:: thetaxbinsumdev(:), thetaybinsumdev(:), thetazbinsumdev(:)   
DOUBLE PRECISION thetaxbinfinal, thetaybinfinal, thetazbinfinal
DOUBLE PRECISION thetaxbindev, thetaybindev, thetazbindev

INTEGER, ALLOCATABLE:: binnk1(:), binnk2(:), binnk3(:), binnk4(:), binnk5(:), binnk6(:), binnk7(:), binnk8(:)    
DOUBLE PRECISION, ALLOCATABLE:: nk1avg(:), nk2avg(:), nk3avg(:), nk4avg(:), nk5avg(:), nk6avg(:), nk7avg(:), nk8avg(:)
DOUBLE PRECISION nk1temp, nk2temp, nk3temp, nk4temp, nk5temp, nk6temp, nk7temp, nk8temp
DOUBLE PRECISION, ALLOCATABLE:: nk1sum(:), nk2sum(:), nk3sum(:), nk4sum(:), nk5sum(:), nk6sum(:), nk7sum(:), nk8sum(:)
DOUBLE PRECISION nk1, nk2, nk3, nk4, nk5, nk6, nk7, nk8
      
INTEGER, ALLOCATABLE:: chainend(:)      
DOUBLE PRECISION  chainbintemp
DOUBLE PRECISION, ALLOCATABLE:: chainsum(:)
...
!Single bead considerations
 ...
 thetaz(k) = 0.D0
  ...
  IF (nw(k) > 1) THEN               
  dxcom(k) = dble(dxmv)/dble(nw(k))
  dycom(k) = dble(dymv)/dble(nw(k))
  dzcom(k) = dble(dzmv)/dble(nw(k))
        
  parasquared = dble(dx*dx) 
  perpsquared = dble(dy*dy + dz*dz)
        
  ete(k) = sqrt(dble(parasquared) + dble(perpsquared))
  etepara(k) = sqrt(dble(parasquared)) 
  eteperp(k) = sqrt(dble(perpsquared)) 
        
  thetax(k) = dble(dx)/ete(k)
  thetay(k) = dble(dy)/ete(k)
  thetaz(k) = dble(dz)/ete(k)
...
!The center of mass used here is the distance to the first bead
  ...
  DO WHILE (atemp /= 13)
   ...
   dxdiff = dble(dx) - dxcom(k) 
   dydiff = dble(dy) - dycom(k)
   dzdiff = dble(dz) - dzcom(k)
...
DO x = 1, nx
 ...
 ELSE
  ...
  nk1avg(x) =  dble(binnk1(x))
  nk2avg(x) =  dble(binnk2(x))
  nk3avg(x) =  dble(binnk3(x))
  nk4avg(x) =  dble(binnk4(x))
  nk5avg(x) =  dble(binnk5(x))
  nk6avg(x) =  dble(binnk6(x))
  nk7avg(x) =  dble(binnk7(x))
  nk8avg(x) =  dble(binnk8(x))
 ENDIF
...
!Static (calculations made every maxsta loops) temp files        
  WRITE(33) x, etebin(x), eteparabin(x), eteperpbin(x), rogbin(x), dble(chainend(x))
